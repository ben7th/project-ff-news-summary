...
def get_vector_data():
    ...
    for item in WebPageEmbeddingItem.objects:
        data.extend(item.line_embeddings)
        for index, _ in enumerate(item.line_embeddings):
            label = {'id': str(item.id), 'line_index': index}
            labels.append(label)
    return data, labels

def __find_best_k(data):
    best_k_elbow, best_k_silhouette, best_k_ch, best_k_db = find_best_k(data, start_k=50, end_k=400)
...
if __name__ == "__main__":
    ...
    # 保存到数据库
    ClusteringResult.objects(name='ff16-kmeans-lines-2023-07-18').update_one(
        upsert=True,
